...
class Anime():
    ...
    def set_url(self, input):
        if re.match(r"^((?:https:\/\/|www.)?myanimelist\.net\/(?:anime)\/([0-9]*)).*", input): # When manga "^((?:https:\/\/|www.)?myanimelist\.net\/(?:anime|manga)\/([0-9]*)).*"
    ...
    def set_id(self):
        match = re.match(r"^((?:https:\/\/|www.)?myanimelist\.net\/(?:anime)\/([0-9]*)).*", self.url) # When manga "^((?:https:\/\/|www.)?myanimelist\.net\/(?:anime|manga)\/([0-9]*)).*"
    ...
    def set_date(self):
        ...
        if match := re.match(r"^(\w{3} [0-9][0-9]?, [0-9]{4})(?: to (\w{3} [0-9][0-9]?, [0-9]{4})?)?", aired):
            date_start = datetime.datetime.strptime(match.group(1), '%b %d, %Y')

            if match.group(2):  # Only have start date, such as movies
                return date_start, datetime.datetime.strptime(match.group(2), '%b %d, %Y')

            return date_start, date_start
        return datetime.datetime.max, datetime.datetime.max,
